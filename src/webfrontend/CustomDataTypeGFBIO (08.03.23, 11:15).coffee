class CustomDataTypeGFBIO extends CustomDataTypeWithCommons
  ...
  __updateSuggestionsMenu: (cdata, cdata_form, gfbio_searchstring, input, suggest_Menu, searchsuggest_xhr, layout, opts) ->
    ...
    setTimeout ( ->
        ...
        # start request
        searchsuggest_xhr.xhr = new (CUI.XHR)(url: location.protocol + '//api.gfbio.fi/rest/v1/search?query=' + gfbio_searchstring + '*&vocab=' + vocParameter + '&lang=' + that.getLanguageParameterForRequests() + '&unique=true&maxhits=' + gfbio_countSuggestions)
            ...
            itemList =
              onClick: (ev2, btn) ->
                ...
                if ! that.renderPopupAsTreeview() || ! that.popover?.isShown()
                  ...
                  allDataAPIPath = location.protocol + '//api.gfbio.fi/rest/v1/data?uri=' + encodeURIComponent(searchUri) + '&format=application%2Fjson'
                  ...
                  dataEntry_xhr.start().done((resultJSON, status, statusText) ->
                    ...
                    allHierarchyAPIPath = location.protocol + '//api.gfbio.fi/rest/v1/' + GFBIOUtilities.getVocNotationFromURI(searchUri) + '/hierarchy?uri=' + encodeURIComponent(searchUri) + '&lang=' + that.getLanguageParameterForRequests() + '&format=application%2Fjson'
                    ...
                    dataHierarchy_xhr.start().done((hierarchyJSON, status, statusText) ->
                      ...
                      # save conceptName
                      cdata.conceptName = GFBIOUtilities.getPrefLabelFromDataResult(resultJSON, desiredLanguage, frontendLanguages)
                      # save conceptURI
                      cdata.conceptURI = resultJSON.uri
                      # save conceptSource
                      cdata.conceptSource = GFBIOUtilities.getVocNotationFromURI(resultJSON.uri)
                      # save _fulltext
  ...
  __getAdditionalTooltipInfo: (uri, tooltip, extendedInfo_xhr, context = null) ->
    ...
    extendedInfo_xhr.xhr = new (CUI.XHR)(url: location.protocol + '//api.gfbio.fi/rest/v1/data?uri=' + uri + '&format=application%2Fjson')
    extendedInfo_xhr.xhr.start()
    .done((data, status, statusText) ->
      htmlContent = GFBIOUtilities.getJSONPreview(data, decodeURIComponent(uri), that.getLanguageParameterForRequests(), ez5.session.getConfigFrontendLanguages())
    ...
    if splittedVocs.length > 1 or splittedVocs == '*'
      select =  {
          ...
          options: (thisSelect) =>
            ...
            searchsuggest_xhr = new (CUI.XHR)(url: location.protocol + '//api.gfbio.fi/rest/v1/vocabularies?lang=' + that.getLanguageParameterForRequests())
            searchsuggest_xhr.start().done((data, status, statusText) ->
                ...
                if ! that.renderPopupAsTreeview()
                  item = (
                    text: $$('custom.data.type.gfbio.modal.form.dropdown.choosefromvocall')
